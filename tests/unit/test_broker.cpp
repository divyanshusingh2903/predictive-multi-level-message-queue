...
TEST(ConfigValidationTest, RejectsInvalidServiceConfiguration) {
    EXPECT_THROW(
        (PMLMQService{PMLMQConfig{.num_levels = 0}}),
        std::invalid_argument);
    EXPECT_THROW(
        (PMLMQService{PMLMQConfig{.default_max_retries = 0}}),
        std::invalid_argument);
    EXPECT_THROW(
        (PMLMQService{PMLMQConfig{.default_ttl = -1ms}}),
        std::invalid_argument);
    EXPECT_THROW(
        (PMLMQService{PMLMQConfig{.max_pull_wait = 0ms}}),
        std::invalid_argument);
}

TEST(ConfigValidationTest, RejectsInvalidAgingConfiguration) {
    EXPECT_THROW(
        (PMLMQService{PMLMQConfig{.aging = AgingConfig{0ms, 100ms}}}),
        std::invalid_argument);
    EXPECT_THROW(
        (PMLMQService{PMLMQConfig{.aging = AgingConfig{100ms, 0ms}}}),
        std::invalid_argument);
}
...
struct LocalBroker {
    PMLMQService service;
    std::unique_ptr<grpc::Server> server;
    std::unique_ptr<pmlmq_rpc::Broker::Stub> stub;
    std::string producer_id;
    std::string consumer_id;

    explicit LocalBroker(PMLMQConfig config) : service(std::move(config)) {
        int port = 0;
        grpc::ServerBuilder b;
        b.AddListeningPort("127.0.0.1:0", grpc::InsecureServerCredentials(), &port);
        ...
        auto ch = grpc::CreateChannel("127.0.0.1:" + std::to_string(port),
                                      grpc::InsecureChannelCredentials());
        stub = pmlmq_rpc::Broker::NewStub(ch);

        grpc::ClientContext c1;
        pmlmq_rpc::RegisterProducerRequest rq1;
        pmlmq_rpc::RegisterProducerResponse rs1;
        EXPECT_TRUE(stub->RegisterProducer(&c1, rq1, &rs1).ok());
        producer_id = rs1.producer_id();

        grpc::ClientContext c2;
        pmlmq_rpc::RegisterConsumerRequest rq2;
        pmlmq_rpc::RegisterConsumerResponse rs2;
...
TEST_F(BrokerTest, PullCapsRequestedWaitAtServerMaximum) {
    LocalBroker b{PMLMQConfig{.max_pull_wait = 100ms}};
...
TEST_F(BrokerTest, UnsetTtlUsesServerDefault) {
    LocalBroker b{PMLMQConfig{.default_ttl = 100ms}};
...
TEST_F(BrokerTest, NackAfterExpiryGoesToDlq) {
    LocalBroker b{PMLMQConfig{.default_ttl = 200ms}};
    b.submit("doomed");
...
TEST_F(BrokerTest, NackBeforeExpiryStillRetries) {
    LocalBroker b{PMLMQConfig{.default_ttl = 5000ms}};
    b.submit("retry-me");
    const auto pulled = b.pull(1000);
    ASSERT_FALSE(pulled.timed_out());
...
TEST_F(BrokerTest, InspectDlqReturnsTtlEntries) {
    ...
    for (const auto& e : resp.entries()) {
        EXPECT_EQ(e.reason(), pmlmq_rpc::DLQ_TTL_EXPIRED);
